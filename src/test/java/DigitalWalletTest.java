...
@DisplayName("Testes da DigitalWallet (JUnit 5) — depósitos, pagamentos, estornos e estados")
class DigitalWalletTest {
    ...
    void constructorSetsInitialBalance() {
        ...
        assertEquals("Carlos", w.getOwner());
        assertFalse(w.isLocked());
        assertFalse(w.isVerified());
    }
    ...
    // 3) Pagamento (@CsvSource) + assumeTrue
    @ParameterizedTest
    @CsvSource({
        "100.0, 30.0, true",
        "50.0, 80.0, false",
    ...
    // 4) Estorno (refund) + assumeTrue + @MethodSource
    @Test
    ...
    // @MethodSource para sequências de estorno
